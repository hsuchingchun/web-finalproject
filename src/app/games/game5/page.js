...
const sketch = (p, handleFinish) => {
  ...
  const countdownImgs = [], scoreboardImgs = [], roundImgs = [];
  ...
  let targetX = 0, targetDir = 1, targetBaseY = 0, targetY = 0, targetAngle = 0;
  ...
  p.draw = () => {
    ...
    if (state === 'instructions') {
      p.image(instructImg, ui.instruction.x, ui.instruction.y, ui.instruction.w, ui.instruction.h);
      p.image(continueImg, ui.continue.x, ui.continue.y, ui.continue.w, ui.continue.h);
    } else if (state === 'countdown') {
      const idx = p.floor((p.millis() - countdownStart) / 1000);
      if (idx < 3) {
        p.image(countdownImgs[idx], ui.countdown.x, ui.countdown.y, ui.countdown.w, ui.countdown.h);
      } else {
        state = 'round';
        roundNumber = 1; score = 0; hasShot = false; bulletActive = false;
        targetX = p.width * 0.8; targetDir = 1; targetAngle = 0;
        targetBaseY = ui.target.y; targetY = targetBaseY;
      }
    } else if (state === 'round') {
      const lbl = ui.roundLabel[roundNumber - 1];
      p.image(roundImgs[roundNumber - 1], lbl.x, lbl.y, lbl.w, lbl.h);
      p.image(scoreboardImgs[score], ui.scoreboard.x, ui.scoreboard.y, ui.scoreboard.w, ui.scoreboard.h);
      ...
      if (!hasShot) {
        p.image(playerStandImg, ui.playerStand.x, ui.playerStand.y, ui.playerStand.w, ui.playerStand.h);
        p.image(shootPromptImg, ui.shootPrompt.x, ui.shootPrompt.y, ui.shootPrompt.w, ui.shootPrompt.h);
      } else {
        p.image(playerShootImg, ui.playerShoot.x, ui.playerShoot.y, ui.playerShoot.w, ui.playerShoot.h);
      }

      if (bulletActive) {
        bulletX += 25; const by = playerY;
        p.image(bulletImg, bulletX, by, ui.bullet.w, ui.bullet.h);
        const regionH=ui.target.h*0.25, cx=targetX, cy=targetY+ui.target.h/2-regionH/2;
        const effR=ui.target.w*0.3, bR=ui.bullet.w;
        if (p.dist(bulletX,by,cx,cy)<=effR+bR) { score++; bulletActive=false; }
      ...
      if (hasShot && !bulletActive) {
        ...
        else { hasShot=false; bulletActive=false; targetX=p.width*0.8; targetDir=1; targetAngle=0; targetY=targetBaseY; }
      }
    } else if (state === 'result') {
      const img = score >= 3 ? successImg : failureImg;
      p.image(img, ui.resultPrompt.x, ui.resultPrompt.y, ui.resultPrompt.w, ui.resultPrompt.h);
      p.image(exitImg, ui.resultBtn.exitX, ui.resultBtn.y, ui.resultBtn.w, ui.resultBtn.h);
      p.image(replayImg, ui.resultBtn.replayX, ui.resultBtn.y, ui.resultBtn.w, ui.resultBtn.h);
    }
  };
